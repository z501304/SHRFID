...
//	USART1_SendData("\x5A\x55\x08\x00\x0D\x11\x00\x00\x01\xD6\x6A\x69",12);
    //----单次盘点------
    USART1_SendData("\x5A\x55\x08\x00\x0D\x11\x00\x00\x00\xD5\x6A\x69",12);
...
void SendSortNumber(u8 NUM)    // 1;2;4;8;
{
    u8 BoxNumber;
    BoxNumber = NUM;
    switch(BoxNumber)
...
// 										break;
            ...
            if((HaveBookFlag == 0x01))/* third in trigger &&  have book*/
            {
                ...
                TimeFlag = 0;
                HaveBookFlag	= 0x00;
                if((SortNum > 0)&&(SortNum < 6))
                {
                    SendSortNumber(SortNum);
                }
                else
                {
                    SendSortNumber(5);
