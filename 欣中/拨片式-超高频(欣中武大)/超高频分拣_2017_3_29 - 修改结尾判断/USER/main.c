 ...
 * @FilePath: main.c
 * @Author: jack
 * @Date: 2023-05-06 10:55:09
 * @LastEditors: Please set LastEditors
 * @LastEditTime: 2023-06-06 13:47:16
 * @Copyright: 2023 xxxTech CO.,LTD. All Rights Reserved.
 * @Descripttion: 增加报文长度32判断
******************************************************************************************/
...
#include "sys.h"
#include "usart.h"
#define    POWER            250    //原来值250
#define    BoardNum         10      //设备编号(从0开始)
#define    Time             1
...
//================== step 1=========================
        if(IN_1 == HAVE_BOOK)/* fist in trigger*/
        {
            ...
            TimeMin  = 0;
            TimeFlag = 0;
            OUT5_OFF; /* start transfer device*/
        }
//================== step 2=========================
...
//			for(k=0;k<100;k++)/*  read 1s */
            while((k>0)&&(HaveBookFlag==0))
            {
                ...
                if(RecvSucceed == 0x01)/* return data*/
                {
                    if(RecvBuf[1] == 0x05)/*  receive tag data*/
                    {
                        if((RecvBuf[28]==0x6a)||(RecvBuf[29]==0x6a)||(RecvBuf[32]==0x6a))
                        {
...
//================== step 3=========================
        if((HaveBookFlag == 0x01))/* third in trigger &&  have book*/
        {
            ...
            TimeFlag = 0;
            HaveBookFlag	= 0x00;
            if( (BoardNum*2+1) == SortNum )//(BoardNum*2+1)
            {
                OUT6_OFF;
                sort_right();
            }
            else if( (BoardNum*2+1) == SortNum )
            {
                OUT6_OFF;
                sort_left();
            }
